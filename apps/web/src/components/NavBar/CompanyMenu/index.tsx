import ZircuitLogo from 'assets/png/zircuit-logo.png'
      ...
      <Popover.Trigger data-testid="nav-company-menu">
        <Trigger>
          <UniIcon onClick={handleLogoClick} data-testid="nav-uniswap-logo">
            {isMobile ? (
              <img src={ZircuitLogo} alt="Zircuit Swap" style={{ height: '48px', width: '48px' }} />
            ) : (
              <NavIcon width="200" height="48" />
            )}
            {isLargeScreen && (
              <Text variant="subheading1" color="$accent3" userSelect="none">
                Zircuit Swap
              </Text>
            )}
          </UniIcon>
